...
#
# RODBC version of the codes
#
# use data files game_log_header.csv
#
# use R packages RODBC, lattice
#
# MySQL installation needed (see book chapter)
# ODBC connection needed (see book chapter)
#
###############################################
...
glheaders <- read.csv("C:\\Users\\sgiraldo\\source\\R\\baseball\\data\\game_log_header.csv")
...
appendGameLogs(start=1871, end=1872, connPackage="RODBC"
               , dsn="rbaseball", uid="user", pwd="password")

# Section 11.5 Querying Data from R

conn <- odbcConnect("RBaseball")
chiAttendance <- sqlQuery(conn, "
...
rockies.games <- sqlQuery(conn, "select year_id, date, parkid
